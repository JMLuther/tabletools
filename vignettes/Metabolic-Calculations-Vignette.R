...
ogtt_long <-
  ...
  pivot_longer(
    ...
    values_to = ".value",
    names_transform = list(time = ~ as.integer(.x))
  ) |> # could be more complicated if not integers
...
pl_summary_glucose <-
  ogtt_long |>
  mutate(
    glucose_tolerance.f = factor_glucose_tolerance(glucose[time == 120])
  ) |>
  ...
  ggplot(aes(
    time,
    glucose_mean,
    group = glucose_tolerance.f,
    color = glucose_tolerance.f
  )) +
  geom_point() +
  geom_line() +
  geom_pointrange(aes(
    ymin = glucose_mean - glucose_sem,
...
ogtt1 <- data.frame(
  ...
  insulin = c(12.8, 30.7, 68.5, 74.1, 44.0)
) # uU/mL
...
## -----------------------------------------------------------------------------
calculate_homa1_ir(ogtt1$glucose[1], ogtt1$insulin[1])
...
ogtt_isi <-
  ogtt_nested |>
  mutate(
    glucose_0 = map_dbl(ogtt_df, ~ .x$glucose[.x$time == 0]),
    insulin_0 = map_dbl(ogtt_df, ~ .x$insulin[.x$time == 0]),
    glucose_fasting.f = factor_glucose_fasting(map_dbl(
      ogtt_df,
      ~ .x$glucose[.x$time == 0]
    )),
    glucose_tolerance.f = factor_glucose_tolerance(map_dbl(
      ogtt_df,
      ~ .x$glucose[.x$time == 0]
    )),
    isi_homa1_ir = map_dbl(
      ogtt_df,
      ~ calculate_homa1_ir(.x$glucose[.x$time == 0], .x$insulin[.x$time == 0])
    ),
    isi_quicki = map_dbl(
      ogtt_df,
      ~ calculate_quicki(.x$glucose[.x$time == 0], .x$insulin[.x$time == 0])
    ), # Uses Log10 c/w Katz publication
    isi_avignon = map2_df(
      ogtt_df,
      weight_kg,
      ~ calculate_isi_avignon(.x$time, .x$glucose, .x$insulin, weight = .y)
    ), # no match
    isi_belfiore = map_dbl(
      ogtt_df,
      ~ calculate_isi_belfiore(.x$time, .x$glucose, .x$insulin)
    ), # no match
    isi_bigtt = pmap_df(
      list(ogtt_df, gender.f, bmi), # Match within rounding error
      ~ calculate_isi_bigtt(
        ..1$time,
        ..1$glucose,
        ..1$insulin,
    ...
    isi_caumo = map2_dbl(
      ogtt_df,
      weight_kg,
      ~ calculate_isi_caumo(.x$time, .x$glucose, .x$insulin, weight = .y)
    ),
    isi_cederholm = map2_dbl(
      ogtt_df,
      weight_kg,
      ~ calculate_isi_cederholm(.x$time, .x$glucose, .x$insulin, weight = .y)
    ), # No match
    isi_gutt = map2_dbl(
      ogtt_df,
      bmi,
      ~ calculate_isi_gutt(.x$time, .x$glucose, .x$insulin, bmi = .y)
    ), # Match
    isi_stumvoll = map_dbl(
      ogtt_df,
      ~ calculate_isi_stumvoll(.x$time, .x$glucose, .x$insulin)
    ), # Match
    isi_stumvoll_dem = pmap_dbl(
      list(ogtt_df, bmi, age), # Match
      ~ calculate_isi_stumvoll_dem(
        ..1$time,
        ..1$glucose,
        ..1$insulin,
        bmi = ..2,
    ...
    isi_matsuda = map_dbl(
      ogtt_df,
      ~ calculate_isi_matsuda(.x$time, .x$glucose, .x$insulin)
    ), # No match
    isi_ogis = pmap_dbl(
      list(ogtt_df, height_cm / 100, weight_kg),
      ~ calculate_isi_ogis(
        ..1$time,
        ..1$glucose,
        ..1$insulin,
    ...
    beta = map_df(
      ogtt_df,
      ~ calculate_stumvoll_beta(.x$time, .x$glucose, .x$insulin)
    ),
    igi_30 = map_dbl(
      ogtt_df,
      ~ calculate_igi30(.x$time, .x$glucose, .x$insulin)
    ),
  )
ogtt_isi |>
  select(
...
ogtt_isi |>
  ...
  unnest() |>
  pivot_longer(
    cols = -c(id, glucose_tolerance.f, glucose_fasting.f),
    names_to = "var",
    values_to = "value"
  ) |>
