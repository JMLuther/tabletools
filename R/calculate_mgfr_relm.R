...
calculate_mgfr_relm <- function(
  ...
  relmipirazin_valid = ifelse(
    tolower(relm_units) %in% c("ng/ml", "mcg/L", "ug/L"),
    T,
    NA
  )
  ...
  # fit model to data
  nonlin2c <- function(t, A, a, B, b) {
    A * (exp(-a * t)) + B * (exp(-b * t))
  }
  fit = nls(
    relmipirazin ~ nonlin2c(time_min, A, a, B, b),
    data = data.frame(time_min, relmipirazin, wt),
    start = list(A = A_start, B = B_start, a = 0.03, b = 0.004),
