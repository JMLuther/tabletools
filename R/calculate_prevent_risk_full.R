...
calculate_prevent_risk_full <- function(
       risk,
       gender,
       age,
       Tc,
       ...
       UACR = ifelse(UACR < 0.1, NA_real_, ifelse(UACR > 25000, NA_real_, UACR))
       HbA1c = ifelse(
              HbA1c < 3.0,
              NA_real_,
              ifelse(HbA1c > 15, NA_real_, HbA1c)
       )
       ...
       # 10 YEAR BASE MODEL
       cf10 <- cfs_full10yr[
              cfs_full10yr$risk == {{ risk_query }} &
                     cfs_full10yr$gender == {{ gender_query }},
       ]
       ...
       # # 30 YEAR BASE MODEL
       cf30 <- cfs_full30yr[
              cfs_full30yr$risk == {{ risk_query }} &
                     cfs_full30yr$gender == {{ gender_query }},
       ]
